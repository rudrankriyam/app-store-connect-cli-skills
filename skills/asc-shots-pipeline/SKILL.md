...
- Framing is pinned to Koubou `0.17.1` for deterministic output.
...
asc cli pins framing to Koubou `0.17.1`.
Install and verify before running framing steps:

```bash
pip install koubou==0.17.1
kou --version  # expect 0.17.1
```
...
- If framing fails with a version error, re-install pinned Koubou: `pip install koubou==0.17.1`.
